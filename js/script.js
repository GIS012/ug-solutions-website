    ...
    if (contactForm) {
        ...
        contactForm.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                if (/needs Activation/i.test(errorMessage)) {
                    setFormStatus('First-time setup: open UGsolution@outlook.com and click the FormSubmit "Activate Form" email link, then send again.', 'error');
                } else if (/open this page through a web server/i.test(errorMessage)) {
                    setFormStatus('Please submit from the live HTTPS site, not from a local HTML file preview.', 'error');
                } else {
                    setFormStatus(errorMessage || 'Message could not be sent. Please try again or email UGsolution@outlook.com.', 'error');
                }
            } finally {
                if (submitBtn) {
